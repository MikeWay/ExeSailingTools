...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="table:table-row[descendant::text:p]">
		<xsl:variable name="durationStr"><xsl:call-template name="calculate-duration"></xsl:call-template></xsl:variable>
		<xsl:variable name="duration" as="xs:dayTimeDuration"
					select="xs:dayTimeDuration($durationStr)" />
		<xsl:variable name="startTimeString"><xsl:call-template name="process-start-time"></xsl:call-template></xsl:variable>

		<event>
			<type>DINGHY</type>
			<description>
				<xsl:value-of select="table:table-cell[1]/text:p" /><xsl:text> </xsl:text>
				<xsl:apply-templates
					select="table:table-cell[position() &gt; $DATE_COL]/text:p" />
			</description>
			<summary>
				<xsl:value-of select="table:table-cell[$EVENT_NAME_COL]/text:p" /><xsl:text> </xsl:text>
				<xsl:call-template name="process-start-location" />
			</summary>
			
			<dateStart>
				<xsl:call-template name="format-date">
					<xsl:with-param name="date-string"><xsl:value-of 
						select="table:table-cell[$DATE_COL]/text:p" /></xsl:with-param>
					<xsl:with-param name="startTimeString" select="$startTimeString" />
					<xsl:with-param name="duration" as="xs:duration"
						select="xs:dayTimeDuration('P0DT0H0M')" />
				</xsl:call-template>
			</dateStart>
	...
	<xsl:template name="format-date">
	    ...
	    <xsl:if test="string-length($startTimeString) &gt; 0">
		    ...
		    <xsl:if test="string-length($day-string) &gt; 0">
		    	...
		    	<xsl:value-of select="format-dateTime($date + $duration, '[H01]:[m01];[D01]/[M01]/[Y0001]')"/>
		    	<sortBy><xsl:value-of select="format-dateTime($date + $duration, '[Y0001][M01][D01]')"/></sortBy> 
		    </xsl:if>
	     </xsl:if>
	    <xsl:if test="string-length($startTimeString) = 0">
		    <xsl:variable name="generated-date-string">2016-<xsl:value-of 
		    ...
		    <xsl:if test="string-length($day-string) &gt; 0">
		    	...
		    	<xsl:value-of select="format-date($date + $duration, '00:00;[D01]/[M01]/[Y0001]')"/>
		    	<sortBy><xsl:value-of select="format-date($date + $duration, '[Y0001][M01][D01]')"/></sortBy>
		    </xsl:if>
	     </xsl:if>	     
